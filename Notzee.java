/* Notzee.java by Dan Seidman
**
**   A java implementation of a dice poker game.
*/
...
public class Notzee extends java.applet.Applet {
	...
	Die dice[] = new Die[5];
	BigButton rollBar, newGameBar, soundToggleBar;
	...
	public void init() {
		...
		soundToggleBar.raised = true;	// not zeroBB'ed in newGame()
		newGame();
		initialized = true;
	}

	// called from init() or resize()
	void initSizes() {
		// initialize dimensions of various features
		Die d;
		...
		// initialize the sound on/off button
		soundToggleBar = new BigButton(this, totalWidth * 3 / 5, totalHeight * 4 / 5, 
			totalWidth / 3, dieSize * 2 / 5);
		...
		scores[11] = new ScoreGroup(this, 11, "Notzee");
	...
	public void paint(Graphics g) {
		...
		// sound on/off bar
		if ((paintFlags & PaintSTBAR) != 0) {
			g.setColor(Color.cyan);
			g.fill3DRect(soundToggleBar.x, soundToggleBar.y, soundToggleBar.width, 
					soundToggleBar.height, soundToggleBar.raised);
			g.setColor(Color.black);
			g.setFont(ngFont);
			g.drawString(soundToggleBar.raised ? "Turn sound off" : "Turn sound on",
					soundToggleBar.x + (soundToggleBar.width / 10), 
					soundToggleBar.y + (fontSize));
		}
	...
	public boolean mouseDown (Event evt, int x, int y) {
		...
		if (rollBar.contains(x, y) && rollBar.active) {
	...
	public boolean mouseUp (Event evt, int x, int y) {
		...
		if (!rollBar.raised && rollBar.contains(x, y)) {
			rollBar.raised = true;
			if (soundToggleBar.raised)
				Aroll.play();
			roll();
			...
			if (dice[0].value == dice[1].value && dice[1].value == dice[2].value &&
			    dice[2].value == dice[3].value && dice[3].value == dice[4].value) {	// Notzee rolled
				Aroll.stop();
		...
		if (!newGameBar.raised && newGameBar.contains(x, y)) {
			newGame();
			paintFlags = PaintALL;
			repaint();
			return true;
		...
		/* sound bar stays down when sound is off */
		if (soundToggleBar.contains(x, y)) {
			soundToggleBar.raised = !soundToggleBar.raised;
			paintFlags = PaintSTBAR;
			repaint();
			return true;
		}

		// if on a valid scoregroup, select and record the score
		if (underMouse != 99 && scores[underMouse].contains(x, y)) {
	...
	public void resize (int x, int y) {
		boolean r = initialized && soundToggleBar.raised;
		boolean a = initialized && rollBar.active;
		super.resize(x, y);
		initSizes(); 
		soundToggleBar.raised = r;
		rollBar.active = a;
	}
...
class Die extends Rectangle {
	...
	Die(Notzee target, int offset) {
		...
		if (parent.initialized) {
			/* on resize, preserve the mutable fields */
			value = parent.dice[offset].value;
			selected = parent.dice[offset].selected;
		} else {
			value = 0;
		}
	}
...
class BigButton extends Rectangle {
	...
	BigButton(Notzee target, int x, int y, int w, int h) {
		super(x, y, w, h);
		parent = target;
		raised = true;
	}
...
class ScoreGroup extends Rectangle {
	...
	ScoreGroup(Notzee target, int offset, String text) {
		x = target.fontSize * 6;
		...
		/* on resize, preserve the mutable fields */
		if (parent.initialized) {
			value = parent.scores[offset].value;
			filled = parent.scores[offset].filled;
			noPaint = parent.scores[offset].noPaint;
		}
	}
	...
	void calc(int groupNum) {
		...
		boolean notzee = false, joker = false, fh;
